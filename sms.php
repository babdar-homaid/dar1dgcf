...
function SMS($USERNAME1, $PASSWORD1, $MobileNo1, $ORIGIN_ADDR1, $TYPE1, $message1,
    $get)
{


  //  $message1 =iconv('Windows-1256', 'UTF-8', $message1);

      $get = @file_get_contents("https://mobile.net.sa/sms/gw/?userName=" . $USERNAME1 ."&userPassword=" . $PASSWORD1 . "&numbers=" . $MobileNo1 ."&userSender=" . $ORIGIN_ADDR1 . "&msg=" . urlencode($message1) . "&By=standard");
}
...
$PASSWORD="6f3e5679d7f4fa98eda7844fc992eb44";

$ORIGIN_ADDR = "School-126"; // اسم المرسل
$TYPE = "ar";

$get1 = @file_get_contents("https://mobile.net.sa/sms/gw/Credits.php?userName=$USERNAME&userPassword=$PASSWORD&By=standard");
"رصيد رسائل الجوال $get1";
...
function SMS($USERNAME1, $PASSWORD1, $MobileNo1, $ORIGIN_ADDR1, $TYPE1, $message1,
    $get)
{
      $get = @file_get_contents("http://www.tqnit.com/smspro/sendsms.php?user=" . $USERNAME1 .
        "&password=" . $PASSWORD1 . "&sender=" . $ORIGIN_ADDR1 . "&numbers=" . $MobileNo1 .
        "&lang=" . $TYPE1 . "&message=" . urlencode($message1) . "");
}
...
$USERNAME="aljouf";
$PASSWORD="57239900";
$ORIGIN_ADDR = "ALSEYIANAH"; // اسم المرسل
$TYPE = "ar";

$get1 = @file_get_contents("http://www.tqnit.com/smspro/sendsms.php?user=" . $USERNAME .
        "&password=" . $PASSWORD . "&action=get");
"رصيد رسائل الجوال $get1";
    ...
    $get)
{
   ...
   $get = @file_get_contents("http://www.mobily.ws/api/msgSend.php?mobile=". $USERNAME1 ."&password=" . $PASSWORD1 . "&numbers=". $MobileNo1 ."&sender=".$ORIGIN_ADDR1."&msg=".convertToUnicode($message1)."&applicationType=24");
}

    


$USERNAME="966548512660";
$PASSWORD="asdasd12";
$ORIGIN_ADDR = "0548512660"; // اسم المرسل

        @SMS($USERNAME, $PASSWORD, $MobileNo100, $ORIGIN_ADDR, $TYPE, $message100, $get);

$get1 = @file_get_contents("http://www.mobily.ws/api/balance.php?mobile=". $USERNAME ."&password=". $PASSWORD ."");
